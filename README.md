...
![LiveForum sample workspace](./docs/product-workspace.png)
...
liveforum actions
liveforum workspace
liveforum page '{"limit":50,"state":"active","search":"Title prefix"}'
liveforum detail '00000000-0000-4000-8000-000000000001'
...
```bash
docker build -t liveforum:0.3.1 .
...
The MCP server uses newline-delimited JSON-RPC over stdio and implements `initialize`, `ping`, `tools/list`, and `tools/call`. It advertises five product utilities, including paginated record list and detail reads, plus the 12 product action tools with their pinned JSON input schemas.
